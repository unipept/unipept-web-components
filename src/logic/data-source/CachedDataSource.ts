...
export abstract class CachedDataSource<T, S extends FAElement> extends DataSource {
    ...
    protected abstract async computeTerms(
        percent: number,
        sequences: string[]
    ): Promise<[Map<T, S[]>, Map<T, FATrust>]>;

    protected async getFromCache(
        namespace: T,
        existingNamespaces: T[],
        cutoff: number = 50, 
        ...
        if ((sequences === null || sequences.length === 0) && cutoff == 50) {
            ...
            const idx: number = this._cachedSequencesLRU.indexOf(sequenceHash);
            ...
            if (idx >= 0) {
                ...
                if (this._cachedSequencesLRU.length > CachedDataSource.MAX_CACHE_SIZE) {
                    const hashToRemove: string = this._cachedSequencesLRU.pop();
